...
//
// Every operation below is a native vector op: abs (synthesised as max(x, -x),
// see gelu_fabs), min, max, mul, add. No division, no elementary-function
// library. research/notes/0001 forbids scalar float in a kernel body (1,617x
// measured), and nothing here is scalar.
//
...
//
// DO NOT use aie::abs() on vector<float> here. On aie2 it resolves through
// aie_api/detail/aie2/abs.hpp's generic path and lowers to the INTEGER
// `llvm.aie2.vabs.gtz32` (checked in the generated LLVM IR and in the core
// disassembly). Integer negation of a float bit pattern is not |x|: for a
// normal float it produces roughly 4/|x| (e.g. -1.0f -> +4.0f, -2.0f -> +2.0f,
// -4.0f -> +1.0f). Measured on hardware: GELU(-1) came out -5.2e-05 instead of
// -1.586e-01, and the whole eltwise output scored 1-cos 9.2e-02 against the
// host path. `max(x, -x)` is |x| using only real float ops (vneg.f + the max
// select) and matches the exact-erf reference.
static inline aie::vector<float, 16> gelu_fabs(const aie::vector<float, 16> &x) {
  return aie::max(x, aie::neg(x));
}
...
#define GELU_HORNER(K)                                                        \
  ...
  for (int i = 0; i < vector_size; i += 64) {
    ...
    aie::vector<float, 16> u0 = aie::min(gelu_fabs(x0), vR);
    aie::vector<float, 16> u1 = aie::min(gelu_fabs(x1), vR);
    aie::vector<float, 16> u2 = aie::min(gelu_fabs(x2), vR);
    aie::vector<float, 16> u3 = aie::min(gelu_fabs(x3), vR);
...
void gelu_epilogue_3072_f32_to_bf16(float *restrict in,
  ...
  for (int i = 0; i < 3072; i += 16) {
    ...
    aie::vector<float, 16> u = aie::min(gelu_fabs(x), vR);
