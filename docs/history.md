# Project history — the dated update blocks, verbatim

**Nothing here has been edited, only moved.** These are the "Update <date>
(tasks/NNNN)" blocks that accumulated at the top of `CLAUDE.md`'s *Current
state* section between 2026-08-18 and 2026-08-22, newest first. By 0075 they
were 1,027 of that file's 1,345 lines — 76% of a file that is loaded into every
session's context, and most of it superseded by the entry above it.

**Why they were kept rather than deleted.** CLAUDE.md rule 3b: *"Failures are
the valuable part — never delete or rewrite them."* These blocks are where the
refuted claims live — the bandwidth cost model, the "missing 4,500 cycles",
B-reuse, the pre-tiling win, the parity-h projection — each with the
measurement that killed it. That record is arguably this repository's most
useful output, and it is worth more here, readable in one pass, than
interleaved with the ground rules an agent needs on every task.

**What replaced them.** `CLAUDE.md` now carries current truth only and points
at three places:

* [`docs/CURRENT_STATUS.md`](CURRENT_STATUS.md) — what works today, with numbers.
* [`research/OPEN-THREADS.md`](../research/OPEN-THREADS.md) — every open
  question, and the authority on what is still open (rule 3).
* [`tasks/`](../tasks/) — the per-session diary the blocks below summarise.

**Read these as history, not as instructions.** Where a block below disagrees
with `docs/CURRENT_STATUS.md` about what the project does *today*, the status
document wins; these say what was true when they were written, which is exactly
their value.

---

**Update 2026-08-22 (tasks/0074): EmbeddingGemma-300M RUNS ON THE ARRAY —
97.7% of its MACs, ~133 seq/s against the host path's 0.20 — and the thing
that had blocked it for nineteen tasks was a question asked about the wrong
option set.** → [`research/OPEN-THREADS.md`](../research/OPEN-THREADS.md) T29
(ANSWERED), T34 (open)

- **The lever is ZERO-PADDING, and it costs nothing.** MQA gives K and V a
  width of 256 (1 KV head × head_dim 256), and 256 caps the legal `tile_n` at
  **16** across the whole design — [`0055`](../tasks/0055-m10-embeddinggemma-spike/TASK.md)
  proved that across all four *fusion* strategies and it is correct. But B is
  **pre-tiled offline anyway**, so appending 256 zero columns to the fused
  Q|K|V takes N from 1280 to **1536**, a multiple of `tile_n·n_aie_cols = 384`.
  Zero columns of B give exactly-zero columns of C, so it is **exact** — there
  is no accuracy question to answer, unlike every other lever on this path.
  Gemma then runs at the production `tile (64,64,48)` and packs to the **same
  `layout_hash`** as MiniLM, bge-small, bge-base and nomic. Cost of the
  padding: **4.4% of GEMM iterations**, against the ~3× it removes.
- **Two errors in the record that this uncovered.** (1) **0055's own table
  already contradicted its verdict** — strategy C is listed there at
  `tile_n = 32`, passing the L1 budget, while the verdict line, the 62–72 seq/s
  prior and the thread's *title* all used 16, the worst strategy, with nothing
  justifying that choice. Taking the worst case out of a table is not the same
  as the table saying so. (2) **`design_fits()` derived `qkv`'s width as
  `3 * hidden`**, which would have made the runtime **reject Gemma's own
  correct design** — the T31 fail-open one field to the left. `qkv_n` is data
  now, in `design.json`, in the container and in the catalogue row.
- **Measured**: 4 GEMMs/layer × 24 layers = **96 dispatches, ONE xclbin, one
  hw_context**; **134.9 / 133.0 / 132.7 seq/s** over three guarded runs (1.7%
  spread, 4 lanes, wall clock end to end — not a kernel claim) against the
  host-only path's **0.20 seq/s** on the same corpus in the same session.
  `1-cos` **9.962e-06** vs that host path, which is itself 5.496e-13 from the
  numpy reference. The **array-only** share is **176 seq/s**, inside the
  140–162 that 0048's cost model predicted *before* the build — the model is
  confirmed at a third width.
- **The strongest correctness statement this project has made about lanes**:
  1, 2, 4 and 6 lanes return **bit-identical** output, and in a 520-row encode
  the 40 repeats of each sentence agree to `max |Δ| = 0`. Batch position, batch
  tier and lane count are provably irrelevant to the answer.
- **Lanes are worth more here than anywhere else** (1.54× against the BERT
  models' 1.19–1.35×) because the array and the host land at **48.9% / 46.0%**
  of wall clock — the most balanced encode in the catalogue. That is also the
  warning: making the host faster partly cannibalises the overlap.
- **A one-row accuracy failure that was the harness.** One row of 520 read
  `1-cos` 6.4e-03 while the other 519 sat at 7e-06 — PowerShell 5.1's
  `Set-Content -Encoding utf8` had written a **UTF-8 BOM**, so `EF BB BF` was
  part of line 0 and the tokenizer correctly embedded a different string. What
  identified it was the *shape*: exactly one row, exactly the first line of the
  file, with the same sentence elsewhere agreeing. A per-row runtime bug does
  not select the first line of a file.
- **Both packers agree byte-for-byte** on the new container (sha256
  `4a4c6f70995601f6…`, first attempt), and the shipped
  `models/embeddinggemma-300m.npue` has that digest — so `--prepare-model` on a
  cold clone now self-produces a container the *array* can run. **The old
  host-only path is kept on purpose** as the discriminating control:
  `--gemma-host-only` packs it and `--cpu` selects it.
- **Still unmeasured, and it is the top item**: **MTEB has never been run on
  this architecture.** At 0.20 seq/s it was impractical; at ~133 it is minutes,
  and [`0035`](../tasks/0035-m8-mteb-gate/TASK.md) makes MTEB — not `1-cos` — the
  authority for accuracy decisions. `run_mteb.py` and `compare_three.py` both
  load models through the BERT-only `npu_encoder.py`, so this model ships with
  a fidelity number, **no quality number, no CPU ratio and no energy figure**.
  `release_benchmark.ps1` writes UNMEASURED with a reason rather than omitting
  the row. → T34
...
→ [`docs/05-measurement/`](05-measurement/README.md)
...
- **M4 — gate PASSED.** `.npue` pre-tiled weight container: round-trip **bit-exact**
  ...
  spec in [`docs/04-model/npue-format.md`](04-model/npue-format.md)
