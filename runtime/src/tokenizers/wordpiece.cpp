//===- wordpiece.cpp ----------------------------------------------------*- C++ -*-===//
//
// NpuEmbeddings -- BERT WordPiece. See tokenizers/wordpiece.hpp.
...
#include "tokenizers/bert_unicode_tables.hpp"
...
void WordPiece::build_index(const std::string &blob) {
...
WordPiece WordPiece::from_vocab_file(const std::string &path) {
...
WordPiece WordPiece::from_vocab_bytes(const char *data, size_t bytes) {
  WordPiece t;
  t.build_index(std::string(data, bytes));
  return t;
}

int32_t WordPiece::id_of(const std::string &token) const {
  auto it = id_of_.find(token);
  return it == id_of_.end() ? unk_id : it->second;
}

const std::string &WordPiece::token_of(int32_t id) const {
...
std::vector<std::string> WordPiece::tokenize_plain(
  ...
  auto flush_word = [&]() {
    ...
    for (const auto &piece : pieces) {
      ...
      // Longer than max_charsPerWord is [UNK] without even trying, which
      // is HuggingFace's behaviour and keeps a pathological input O(1).
      if (static_cast<int>(piece.size()) > max_charsPerWord_) {
        out.push_back("[UNK]");
        continue;
      }
...
Encoded WordPiece::encode(const std::string &text, int max_len) const {
  Encoded e;
...
std::vector<Encoded> WordPiece::encode_batch(
