...
// tokenizers/gemma.hpp for the pipeline summary and the design rationale.
//
// THE MERGE ALGORITHM
// --------------------
...
std::vector<std::string> GemmaTokenizer::tokenize(const std::string &text) const {
  const std::vector<int32_t> ids = tokenize_ids(text);
  std::vector<std::string> out;
  out.reserve(ids.size());
  for (int32_t id : ids) {
    const std::string &tok = token_of(id);
    if (!tok.empty()) out.push_back(tok);
  }
  return out;
}

std::vector<int32_t> GemmaTokenizer::tokenize_ids(const std::string &text) const {
...
GemmaEncoded GemmaTokenizer::encode(const std::string &text, int max_len,
  ...
  const std::vector<int32_t> body = tokenize_ids(full);
...
Encoded GemmaTokenizer::encode(const std::string &text, int max_len) const {
  GemmaEncoded ge = encode(text, max_len, default_prefix_name());
  Encoded e;
  e.input_ids = ge.input_ids;
  e.attention_mask = ge.attention_mask;
  e.n_tokens = ge.n_tokens;
  e.n_tokens_full = ge.n_tokens_full;
  e.truncated = ge.truncated;
