...
#include "tokenizers/xlmr_unicode_tables.hpp"
...
std::vector<int32_t> XlmrTokenizer::encode_raw(const std::string &text) const {
  std::vector<int32_t> ids;
...
Encoded XlmrTokenizer::encode(const std::string &text, int max_len) const {
  std::vector<int32_t> full = encode_raw(text);
  Encoded e;
  ...
  e.input_ids.resize(static_cast<size_t>(max_len), pad_id);
...
std::vector<std::string> XlmrTokenizer::tokenize(const std::string &text) const {
  // XLM-R does not have a literal special-token pass like WordPiece.
  // Return the raw pieces from the Viterbi output.
  std::vector<int32_t> ids = encode_raw(text);
  std::vector<std::string> out;
  out.reserve(ids.size());
  for (int32_t id : ids) {
    const std::string &piece = piece_of(id);
