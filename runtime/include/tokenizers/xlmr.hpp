...
// family). SPDX-License-Identifier: Apache-2.0
//
// NOT WordPiece (tokenizers/wordpiece.hpp) and NOT SentencePiece BPE
// (tokenizers/gemma.hpp): this checkpoint's tokenizer.json declares
...
class XlmrTokenizer : public Tokenizer {
public:
  ...
  // <s> ids </s>. No padding, no truncation (0110: an input that does
  // not fit is the CALLER's error to raise, not this class's to hide).
  std::vector<int32_t> encode_raw(const std::string &text) const;

  // Base class encode(): applies padding/truncation to the raw output.
  Encoded encode(const std::string &text, int max_len) const override;

  size_t vocab_size() const override { return pieces_.size(); }
