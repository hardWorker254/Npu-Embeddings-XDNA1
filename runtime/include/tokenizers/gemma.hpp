...
//
// NOT WordPiece (see tokenizers/wordpiece.hpp for that). This checkpoint's own
...
public:
  ...
  std::vector<int32_t> tokenize_ids(const std::string &text) const;
  ...
  // Base class encode(): converts GemmaEncoded to Encoded (no prefix).
