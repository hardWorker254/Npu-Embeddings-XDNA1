...
// Set to 1 by the SIGINT/SIGTERM handler that serve_http installs. Server::run
// polls it (and between interrupted select()s) so an interrupt returns through
// the C++ stack, destroying the Device and every Design and releasing their
// hw_contexts -- instead of the process dying with the contexts still held,
// which is what makes the NEXT run fail (SUBTASKS.md subtask 7). Defined in
// src/server/embed_backend.cpp.
extern volatile std::sig_atomic_t g_server_stop;
...
public:
  ...
  // Blocks until g_server_stop is set, then returns so the caller's stack
  // unwinds and releases the NPU contexts. `handler` returns
  ...
  void run(H &&handler) {
    for (;;) {
      if (g_server_stop) {
        std::fprintf(stderr,
                     "\n  interrupted -- releasing NPU contexts\n");
        return;
      }
      // A bounded select() rather than a bare accept(): a signal delivered
      // while we wait must not leave the loop blocked in accept() with the
      ...
      const int ready = select(static_cast<int>(listen_) + 1, &rd, nullptr,
                               nullptr, &tv);
      if (ready < 0) continue;    // EINTR (signal) or error: recheck the flag
      if (ready == 0) continue;   // timeout: recheck the flag
