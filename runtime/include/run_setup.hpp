...
inline int setup_encoder(RunContext &ctx) {
  ...
  // Build the encoder on the host side of the designs. The model and mask
  // are references -- they outlive the encoder via RunContext.
  ctx.enc = std::make_unique<Encoder>(
      Encoder{(*ctx.model), ctx.d_qkv(), ctx.d_ao(), ctx.d_fu(), ctx.d_fd(),
              ctx.d_gelu(), ctx.d_ln(), ctx.d_sm(), ctx.mask});
  ...
  if (ctx.sim_c_bf16) {
    ...
    if (ctx.enc->qkv.info().a_elem_bytes != 1) {
      std::fprintf(stderr,
                   "--sim-c-bf16 is only meaningful on an int8 design "
                   "(this one carries %d-byte operands)\n",
                   (int)ctx.enc->qkv.info().a_elem_bytes);
      return 2;
    }
    if (ctx.enc->qkv.info().c_elem_bytes == 2) {
      std::fprintf(stderr,
  ...
  if (ctx.pipeline > 1) {
    ...
    for (int l = 1; l < ctx.pipeline; ++l) {
      ctx.lanes.push_back(std::make_unique<Encoder>(
          Encoder{(*ctx.model), ctx.d_qkv(), ctx.d_ao(), ctx.d_fu(), ctx.d_fd(),
                  ctx.d_gelu(), ctx.d_ln(), ctx.d_sm(), ctx.mask}));
      Encoder &e2 = *ctx.lanes.back();
      e2.batch = ctx.batch;
      e2.rows = ctx.rows;
      e2.pool = ctx.pools[l].get();
      e2.unified = true;
